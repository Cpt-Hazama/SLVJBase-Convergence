...
function ENT:LookForTarget()
	...
	local entTgt
	for _, ent in ipairs(self:FindInCone(35,8000,function(ent) return (ent:IsNPC() || (ent:IsPlayer() && (self.entOwner:IsPlayer() && gamemode.Call("CanPlayerDamagePlayer",ent,self.entOwner) || self.entOwner:IsNPC()))) && ent:Alive() && (!ent:IsNPC() || ent:Disposition(self.entOwner) == D_HT) && self:Visible(ent) end)) do
